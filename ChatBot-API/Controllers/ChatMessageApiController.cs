...
using ChatBot_API.UnitOfWork;
using ChatBot_API.Tavily;
...
namespace ChatBot_API.Controllers
{
    ...
    public class ChatController : ControllerBase
    {
        ...
        public async Task<IActionResult> SendMessage([FromBody] ChatMessageRequestDto request)
        {
            ...
            try
            {    
                await _unitOfWork.Repository<ChatMessage>().AddAsync(userMessage);
        ...
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            var messages = await _unitOfWork.Repository<ChatMessage>().GetAsync(cm => cm.UserId == userId, page,pageSize);
            return Ok(messages);
        }
        ...
        {
            var message = await _unitOfWork.Repository<ChatMessage>().GetByIdAsync(id);
            ...
            await _unitOfWork.Repository<MessageEdit>().AddAsync(edit);
            await _unitOfWork.SaveAsync();
            return Ok(message);
        }
